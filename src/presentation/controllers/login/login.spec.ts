...
describe('Login Controller', () => {
  test('Should return 400 if no email is provided', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest({ email: undefined }))
    expect(httpResponse).toEqual(badRequest(new MissingParamError('email')))
  })

  test('Should return 400 if no password is provided', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest({ password: undefined }))
  ...
  test('Should return 400 if an invalid email is provided', async () => {
    ...
    const httpResponse = await sut.handle(makeFakeRequest({ email: 'invalid_email' }))
